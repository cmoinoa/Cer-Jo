...
//-----------------------------------------------------------------
//-----------------------------------------------------------------
void choixdate(int * mois, int * jour){
...
//-----------------------------------------------------------------
//-----------------------------------------------------------------
float t_moyenne(float *temps, int taille) {
...
void relais(char nom[nb_sportif][25]) {
  ...
  for (int i = 0; i < 4; i++) {
    do {
      ...
      for (int j = 0; j < i; j++) {
        if (listerelais[j] == listerelais[i]) {
          printf("\n-------------------------------\n\n\033[0;31m# Athlete déjà séléctionné, choississez-en un différent\033[0m\n");
          verifdiff = 1;
        }
      }
      maj_athlete(nom, listerelais[i], 5, mois, jour, temps, 1, 1);
    } while (verifdiff == 1);
    fprintf(fichier, "24/%d/%d %ds %s %d\n", mois, jour, temps,
            nom[listerelais[i]], i + 1);
  }
  fprintf(fichier, "----------------------------------------\n");
  fclose(fichier);
...
//-----------------------------------------------------------------
//-----------------------------------------------------------------
void menu(char nom[nb_sportif][25]) { // premier choix de l'interface
int verif = 1;
while (verif) {
    int choix;
    printf("\n-------------------------------\n\nMenu principal\n\n1 : Entrainement\n2 : Relais\n3 : Historique\n4 : Statistique\n\nChoix : ");
...
//-----------------------------------------------------------------
//-----------------------------------------------------------------
void creafichier() {//crée un fichier pour chaque athlète
